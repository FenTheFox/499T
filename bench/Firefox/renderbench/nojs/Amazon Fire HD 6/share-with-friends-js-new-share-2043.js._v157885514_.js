...
return true;}
var updateHrefs=function(elementId){var element=jQuery('#'+elementId);var href=jQuery(element).attr('href');if(href.indexOf("childASIN")>=0){href=href.replace(/childASIN=.*/g,"childASIN="+selectedASIN);}else{href+="&childASIN="+selectedASIN;}
...
var updateEventsForShare=function(){for(triggerID in amz_taf_triggers){if(amz_taf_triggers&&amz_taf_triggers[triggerID]&&amz_taf_triggers[triggerID].isCompact){jQuery('#'+triggerID).removeAmazonPopoverTrigger();amz_taf_bindCompactPopoverTrigger(triggerID);var hiddenDivID="tafPopHidden_"+triggerID;jQuery('#'+hiddenDivID).removeAmazonPopoverTrigger();amz_taf_bindLightBoxTrigger(hiddenDivID,selectedASIN,parentASIN);updateHrefs("tafCompact_"+triggerID);}
amz_taf_bindLightBoxTrigger(triggerID,selectedASIN,parentASIN);updateHrefs(triggerID);updateSocialNetworkLinks(triggerID);}}
...
amz_taf_triggers[triggerID].url=amz_taf_triggers[triggerID].url+"&redirectLocation="+url;amz_taf_triggers[triggerID].origTriggerID=triggerID;var isCompact=amz_taf_triggers[triggerID].isCompact;var hiddenDivID="tafPopHidden_"+triggerID;if(isCompact&&amz_taf_triggers[hiddenDivID]){amz_taf_triggers[hiddenDivID].url=amz_taf_triggers[hiddenDivID].url+"&redirectLocation="+url;amz_taf_triggers[hiddenDivID].origTriggerID=triggerID;jQuery("#"+triggerID).after('<div id="'+hiddenDivID+'" style="display:none"></div>');amz_taf_bindCompactPopoverTrigger(triggerID);amz_taf_bindLightBoxTrigger(hiddenDivID);}
amz_taf_bindLightBoxTrigger(triggerID);if(isRedirect&&!amz_taf_showSwfCalled){amz_taf_showSwfCalled=true;jQuery("#"+triggerID).trigger('click');}
...
popoverHTML+="<li style='padding:6px 9px;margin:0px !important;font-size:11px;line-height:16px;"+border+"'>";var networkHTML="<span class='"+network.className
+"' style='"+network.style
+";margin:0px !important;'></span><span class='tafShareText' style='margin:0px;'>"
...
jQuery("#span_"+triggerID).amazonPopoverTrigger({literalContent:popoverHTML,showOnHover:true,cacheable:false,hoverShowDelay:50,followLink:true,location:'bottom',locationMargin:12,align:'right',width:134,hoverHideDelay:300,showCloseButton:false,closeEventInclude:'CLICK_TRIGGER',group:'taf'});}
function amz_taf_unbindCompactPopoverTrigger(triggerID){jQuery("#span_"+triggerID).removeAmazonPopoverTrigger();}
...
amz_taf_popover=popover;amz_taf_popover.processing=0;jQuery(document).keyup(function(evt){if(27==evt.which&&amz_taf_popover.css('visibility')!='hidden'&&amz_taf_popover.css('display')!='none'){amz_taf_popover.hide();}});amz_taf_unbindCompactPopoverTrigger(amz_taf_triggers[triggerID].origTriggerID);}
...
var postContent={};var count=0;var recipients=recipientsObj.value;if(recipients!=amz_taf_config.detailedInstructions){recipients=recipients.replace(/^(,|;|\s)+/,"");recipients=recipients.replace(/(,|;|\s)+$/,"");if(recipients!=""){postContent.recipients=recipients;}}
...
function amz_taf_friends_show(show){(function($){if(show){$('.tafPoContent .checkBoxArea').show();$('.tafPoContent .manipCheckBoxArea a.show').hide();$('.tafPoContent .manipCheckBoxArea a.hide').show();$('.tafPoContent .manipCheckBoxArea a.check').show();}else{$('.tafPoContent .checkBoxArea').hide();$('.tafPoContent .manipCheckBoxArea a.show').show();$('.tafPoContent .manipCheckBoxArea a.hide').hide();$('.tafPoContent .manipCheckBoxArea a.check').hide();}})(jQuery);}
function amz_taf_friends_check_toggle(){(function($){var boxes=$('input[name="friendsRecipients"]');boxes.attr('checked',!boxes.is(':checked'));})(jQuery);}
...
$('form[name="tellAFriendForm"] input:hidden[name="channel"]').val('email');amz_taf_sendButton_onSubmit($('.tafPoContent form')[0]);});$('form[name="tellAFriendForm"]').submit(function(){$(this).find('input:hidden[name="channel"]').val('email');amz_taf_sendButton_onSubmit(this);return false;});})(jQuery);};amznJQ.declareAvailable('share-with-friends-js-new');
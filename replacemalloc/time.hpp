...
public:
	...
	double duration()
	{
		if (start_time == steady_clock::time_point::min() || end_time == steady_clock::time_point::min())
			std::cout << "Please call both start() and end() in between each call to duration()" << std::endl;
	...
	uint64_t hwduration()
	{
		if (hwstart == 0 || hwend == 0)
			std::cout << "Please call both start() and end() in between each call to hwduration()" << std::endl;

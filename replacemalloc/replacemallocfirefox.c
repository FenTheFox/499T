...
{
	return macwrapper_malloc(size);
}

void *replace_calloc(size_t num, size_t size)
{
	return macwrapper_calloc(num, size);
}

void *replace_realloc(void *ptr, size_t size)
{
	return macwrapper_realloc(ptr, size);
}
...
{
	return macwrapper_memalign(alignment, size);
}

int replace_posix_memalign(void **ptr, size_t alignment, size_t size)
{
	return macwrapper_posix_memalign(ptr, alignment, size);
}
...
{
	return macwrapper_valloc(size);
}

size_t replace_malloc_usable_size(usable_ptr_t ptr)
{
	return macwrapper_malloc_usable_size(ptr);
}

size_t replace_malloc_good_size(size_t size)
{
	return macwrapper_malloc_good_size(size);
}